...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {ratingPromptMovie && <RatingPrompt movie={ratingPromptMovie} value={ratings[ratingPromptMovie.id]} onRate={(value) => rateMovie(ratingPromptMovie, value)} onDismiss={() => setRatingPromptMovie(null)} />}
  ...
  return <section className={props.hasSubmittedPrompt ? "pick-page page-enter" : "pick-page is-empty page-enter"}>
    ...
    <div className={props.expandedResults ? "pick-stage is-expanded" : "pick-stage"} aria-live="polite">
      {props.loading ? <>{[1, 2, 3].map((rank) => <div className={`pick-card skeleton-card rank-${rank}`} key={rank}><div className="poster-skeleton"/><div className="line-skeleton"/><div className="button-skeleton"/></div>)}</> : props.expandedResults
        ...
        : props.results.map((result, index) => <RecommendationCard key={result.movie.id} result={result} rank={index + 1} expanded={props.expandedReason === result.movie.id} onExpand={() => props.setExpandedReason(props.expandedReason === result.movie.id ? null : result.movie.id)} onWatched={() => props.onWatched(result.movie)} onSave={() => props.onSave(result.movie)} onPick={() => props.onPick(result)} onReject={() => props.onReject(result.movie)} saved={Boolean(props.watchlist[result.movie.id])} developerMode={props.developerMode} />)}
    </div>
...
function RecommendationCard({ result, rank, expanded, saved, onExpand, onWatched, onSave, onPick, onReject, developerMode }: { result: RecommendationResult; rank: number; expanded: boolean; saved: boolean; onExpand: () => void; onWatched: () => void; onSave: () => void; onPick: () => void; onReject: () => void; developerMode: boolean }) {
...
function RatingPrompt({ movie, value, onRate, onDismiss }: { movie: Movie; value?: number; onRate: (rating: number) => void; onDismiss: () => void }) {
  return <aside className="rating-toast" aria-live="polite"><img src={posterUrl(movie.posterPath, "w92")} alt=""/><div><strong>How was {movie.title}?</strong><StarRating value={value} onChange={onRate} /></div><button className="toast-close" onClick={onDismiss} aria-label={value ? "Done rating" : "Skip rating"}>×</button></aside>;
}
